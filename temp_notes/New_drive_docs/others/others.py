# region STOCKS
# -------------
'''
1. find the sector which is growing the fastest or may demand in the future
2. figure out all the stocks that are available in tha sector
3. find which stocks are overvalued or undervalued
🔗 https://www.gurufocus.com/dcf-calculator
4. find out the fundamentals using below links
🔗 https://www.screener.in/
🔗 https://www.moneycontrol.com/india/stockpricequote/pharmaceuticals/lauruslabs/LL05
5. compare each stocks with each other of same sectors
6. use UP20 Strategy

# DONOT invest more than 20% of the income of the salary
# DONOT invest more than 9% of the the investing fund in 1 company
1 lath - 9% - 9000 - split into 3 - 3000 make 3 times

'''



perplexity.ai for research


grok (Deepsearch, grok 3) - financial anlysis / compare stocks

right price entry - Stock Name : NTPC Author Name : Himanshu Rai , consice, claude sonnet 4
claude.ai
# endregion